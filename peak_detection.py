...
# 입력 경로 (baseline / holding)
BASELINE_PATH = r"C:\path\to\baseline_ir.txt"
HOLDING_PATH  = r"C:\path\to\holding_ir.txt"
...
def compare_groups_boxplot(df_base: pd.DataFrame, df_hold: pd.DataFrame, title='IR'):
    ...
    # Welch t-test (표출은 선택적)
    pvals = {}
    for f in feats:
        a = df_base[f].dropna().values; b = df_hold[f].dropna().values
    ...
    for i, f in enumerate(feats):
        ax = axes[i]
        x = [df_base[f].dropna().values, df_hold[f].dropna().values]
        bp = ax.boxplot(x, patch_artist=True, labels=['baseline','holding'])
        ...
        p = pvals.get(f, np.nan)
        suffix = '' if np.isnan(p) else (f' (p={p:.3g})')
        ax.set_title(f'{f}{suffix}')
        ax.grid(True, linestyle='--', alpha=0.4)
    axes[-1].axis('off')  # 9번째 칸 비우기
